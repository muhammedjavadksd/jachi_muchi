...
export const Step07_AstigmatismRight = memo(function Step07_AstigmatismRight() {
  ...
  useEffect(() => {
    ...
    // Draw sunburst lines every 10 degrees
    for (let deg = 0; deg < 360; deg += 10) {
      ...
      // Vary line width slightly to simulate astigmatism effect
      const variation = Math.sin(deg * 0.3) * 0.5 + 1;
      ctx.beginPath();
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
  ...
  const handleAllEqual = () => {
    dispatch({ type: "SET_RIGHT_ASTIGMATISM", payload: { result: "none", axis: null } });
    setAllEqual(true);
    setDone(true);
  };
  ...
  const handleSomeDarker = () => {
    setAllEqual(false);
    // In a full version we'd let user click, for now approximate
    dispatch({ type: "SET_RIGHT_ASTIGMATISM", payload: { result: "mild", axis: 90 } });
    setDone(true);
  };

  const handleContinue = () => dispatch({ type: "SET_STEP", payload: 8 });
    ...
    return (
      <StepWrapper>
        <div className="flex flex-col items-center text-center max-w-lg mx-auto px-4 py-12">
          ...
          <h3 className="text-lg font-bold text-[#05005B] mb-2">Right Eye Astigmatism Check</h3>
          ...
          <button onClick={handleContinue} className="w-full max-w-sm py-3.5 px-6 bg-[#05005B] hover:bg-[#0a0a7a] text-white font-semibold rounded-xl transition-all text-sm">
            Continue to Left Eye Astigmatism
          </button>
        </div>
      </StepWrapper>
    );
      ...
      <div className="flex flex-col items-center max-w-lg mx-auto px-4 py-6 sm:py-8 text-center">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-5 w-full text-left">
          <p className="text-xs sm:text-sm text-amber-800 font-medium">
            ✋ Cover your <strong>LEFT</strong> eye. Look at the image below.
          </p>
        </div>

        <h2 className="text-lg sm:text-xl font-bold text-[#05005B] mb-3">Astigmatism Test — Right Eye</h2>
