...
const AddAddressModal = memo(function AddAddressModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white w-full sm:rounded-xl sm:max-w-lg sm:mx-4 max-h-[92vh] overflow-y-auto rounded-t-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-gray-200 sticky top-0 bg-white z-10">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Add New Address</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5">
            ...
            <div className="flex gap-2 sm:gap-3">
              {(["HOME", "OFFICE", "OTHER"] as const).map((type) => (
                <button
                  ...
                  className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                    formData.type === type
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4">
            <div>
              ...
              <input
                ...
                onChange={handleInputChange}
                className="w-full px-3 sm:px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm"
                placeholder="Pincode"
                maxLength={6}
                required
              />
          ...
          <button
            type="submit"
            className="w-full py-4 bg-teal-700 text-white font-semibold rounded-lg hover:bg-teal-800 transition-colors text-sm sm:text-base"
          >
            Save Address
          </button>
        </form>
...
export const CheckoutPage = memo(function CheckoutPage(): JSX.Element {
  ...
  const handleAddAddress = useCallback(
    (addressData: Omit<Address, "id" | "isSelected" | "deliveryDate">) => {
  ...
  /** Progress steps — compact on mobile (icons only for inactive) */
  const progressSteps = useMemo(() => (
    <div className="flex items-center overflow-x-auto hide-scrollbar gap-1 sm:gap-0 mb-6 sm:mb-8">
      {CHECKOUT_STEPS.map((step, index) => (
        <div key={step.id} className="flex items-center shrink-0">
          ...
          {index < CHECKOUT_STEPS.length - 1 && (
            <svg className="w-3 h-3 sm:w-4 sm:h-4 mx-1 sm:mx-3 text-gray-300 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const addressCards = useMemo(() => (
    addresses.map((address) => (
      <div
        key={address.id}
        className={`bg-sky-50 border-2 rounded-lg p-4 sm:p-5 mb-4 cursor-pointer transition-colors ${
      ...
      >
        <div className="flex items-start justify-between mb-3">
          ...
          >
            {address.isSelected && <div className="w-2 h-2 bg-white rounded-full" />}
          </div>
        </div>

        <p className="text-gray-900 font-medium mb-1 text-sm sm:text-base">{address.fullAddress}</p>
        <p className="text-gray-700 mb-3 text-sm">{address.name}</p>

        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-gray-500 text-xs sm:text-sm mb-4">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          <span className="text-gray-600 text-xs sm:text-sm">Get it by {address.deliveryDate}</span>
          <div className="flex items-center gap-3 sm:gap-4">
            <button
              onClick={(e) => { e.stopPropagation(); handleDeleteAddress(address.id); }}
              className="text-gray-500 text-xs sm:text-sm font-medium hover:text-gray-700 underline"
            >
              Delete
            </button>
            <button
  ...
  const billPanel = (
    <>
      {/* Savings Banner */}
      <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4 mb-4 flex items-start sm:items-center gap-2">
        <svg className="w-5 h-5 text-green-600 shrink-0 mt-0.5 sm:mt-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <Container>
          ...
          <div className="hidden lg:flex gap-8">
            ...
            <div className="flex-1 min-w-0">
              <button
                onClick={openModal}
                className="flex items-center justify-center gap-2 w-full max-w-md mx-auto mb-6 px-6 py-3 bg-teal-700 text-white font-medium rounded-full hover:bg-teal-800 transition-colors text-sm"
              >
          ...
          <div className="lg:hidden">
            <button
              onClick={openModal}
              className="flex items-center justify-center gap-2 w-full mb-5 px-6 py-3 bg-teal-700 text-white font-medium rounded-full hover:bg-teal-800 transition-colors text-sm"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
              </svg>
            ...
            {!showBill && (
              <button
                type="button"
                onClick={() => navigate("/order-success")}
                className="w-full py-4 bg-teal-700 text-white font-semibold rounded-lg hover:bg-teal-800 transition-colors text-sm"
              >
                Save Address & Proceed · ₹{BILL_SUMMARY.totalPayable}
              </button>
            )}
          </div>
        </Container>
