...
export const Step10_SphereLeft = memo(function Step10_SphereLeft() {
  ...
  const handleConfirm = useCallback(() => {
    dispatch({ type: "SET_LEFT_SPHERE", payload: power });
    setDone(true);
  }, [power, dispatch]);
    ...
    return (
      <StepWrapper>
        <div className="flex flex-col items-center text-center max-w-lg mx-auto px-4 py-12">
          ...
          <h3 className="text-lg font-bold text-[#05005B] mb-2">Left Eye Sphere Recorded</h3>
          <p className="text-gray-500 text-sm mb-4">Sphere: <strong>{power >= 0 ? "+" : ""}{power.toFixed(2)}</strong></p>
          <button onClick={() => dispatch({ type: "SET_STEP", payload: 11 })} className="w-full max-w-sm py-3.5 px-6 bg-[#05005B] hover:bg-[#0a0a7a] text-white font-semibold rounded-xl transition-all text-sm">
      ...
      <div className="flex flex-col items-center max-w-lg mx-auto px-4 py-6 sm:py-8 text-center">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-5 w-full text-left">
          <p className="text-xs sm:text-sm text-amber-800 font-medium">
            ✋ Cover your <strong>RIGHT</strong> eye. Adjust the slider until the letter appears sharp and clear.
          </p>
        </div>

        <h2 className="text-lg sm:text-xl font-bold text-[#05005B] mb-4">Sphere Power — Left Eye (Distance)</h2>
