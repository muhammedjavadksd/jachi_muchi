...
export const TopCategories = memo(function TopCategories(): JSX.Element {
  ...
  const categoryCards = useMemo(() => (
    categories.map((category: any) => {
      ...
      const hasDropdown = matchedCategory?.shapes?.length > 0;
      return (
        <div key={category.name} className="relative group">
          ...
          >
            ...
            >
              <img src={category.image} alt={category.name} className="w-4/5 h-4/5 object-contain" loading="lazy" />
              {category.badge && (
          ...
          {hasDropdown && hoveredCategory === category.name && (
            ...
            >
              <div className="py-2">
                {matchedCategory?.shapes.map((shape: string, idx: number) => (
                  <button
                    key={idx}
                    onClick={() =>
                      navigate(`/search/${matchedCategory.slug}?shape=${shape}`)
                    }
              ...
              <div className="border-t border-gray-100">
                <button onClick={() => navigate(`/category/${matchedCategory.slug}`)} className="w-full py-2.5 text-center text-sm font-medium text-teal-600 hover:text-teal-700 hover:bg-gray-50 transition-colors">
