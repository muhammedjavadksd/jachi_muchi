...
export const SecondaryBannerCarousel = memo(function SecondaryBannerCarousel(): JSX.Element {
  ...
  // useEffect(() => {
  //   const id = window.setInterval(goNext, AUTO_SCROLL_INTERVAL);
  //   return () => window.clearInterval(id);
  // }, [goNext]);

  /** One item + one gap so scroll shows next set; gap-4 = 1rem */
  // const trackStyle = useMemo(
  //   () => ({
  //     transform: `translateX(calc(-${index} * ((100% - 3rem) / 4 + 1rem)))`,
  //   }),
  //   [index]
  // );
  ...
  <div className="flex gap-3 sm:gap-4 overflow-x-auto scrollbar-hide">
    {SECONDARY_BANNERS.map((src, i) => (
      ...
      >
        ...
        >
          <span className="inline-block -translate-x-[1px]">&lsaquo;</span>
        </button>
        <button
          type="button"
          onClick={goNext}
        ...
        >
          <span className="inline-block translate-x-[1px]">&rsaquo;</span>
        </button>
      </div>
    </section>
  );
