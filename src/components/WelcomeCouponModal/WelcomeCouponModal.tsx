...
import { fetchWelcomeCoupon, type WelcomeCoupon } from "../../lib/couponApi";
...
export function WelcomeCouponModal({
  ...
  const handleMaybeLater = () => {
    // Still save coupon for later use on checkout
    if (welcomeData) {
      localStorage.setItem("welcomeCoupon", JSON.stringify({
        code: welcomeData.code,
        discount: welcomeData.discountType === 'percentage' ? `${welcomeData.discountValue}%` : `₹${welcomeData.discountValue}`,
        ...
        validDays: welcomeData.validDays,
        applied: false,
        skipped: true, // Mark as skipped so checkout doesn't auto-apply immediately
      }));
    }
    onClose();
  };
